const state = { token: localStorage.getItem('adminToken'), products: [], orders: [], activeTab: 'products' };
...
function renderOrders(){
  $('#orderList').innerHTML=state.orders.length?state.orders.map(order=>`<article class="order"><div><strong>#${order.id.slice(0,8)} · NT$ ${order.total}</strong><p>${escapeHtml(order.summary)}</p><p>${escapeHtml(order.customer_name||'LINE 客戶')} · ${escapeHtml(order.phone||'未留電話')} · ${order.fulfillment==='delivery'?'外送':'到店取貨'}${order.pickup_time?` · ${escapeHtml(order.pickup_time)}`:''}</p><p>付款：${order.payment_method==='bank_transfer'?'銀行轉帳':'現金取貨'}${order.transfer_last5?` · 末五碼 ${escapeHtml(order.transfer_last5)}`:''}</p>${order.note?`<p>備註：${escapeHtml(order.note)}</p>`:''}<small>${new Date(order.created_at).toLocaleString('zh-TW')}</small></div><div class="order-actions"><select data-type="order" data-id="${order.id}">${Object.entries(statusLabels).map(([value,label])=>`<option value="${value}" ${order.status===value?'selected':''}>${label}</option>`).join('')}</select><select data-type="payment" data-id="${order.id}">${Object.entries(paymentLabels).map(([value,label])=>`<option value="${value}" ${order.payment_status===value?'selected':''}>${label}</option>`).join('')}</select></div></article>`).join(''):'<div class="empty">目前還沒有訂單。</div>';
  document.querySelectorAll('.order select').forEach(select=>select.addEventListener('change',async()=>{const payment=select.dataset.type==='payment';try{await api(`/api/admin/orders/${select.dataset.id}/${payment?'payment':'status'}`,{method:'PATCH',body:JSON.stringify(payment?{payment_status:select.value}:{status:select.value})});toast(payment?'付款狀態已更新':'訂單狀態已更新並通知客戶');}catch(error){toast(error.message);loadOrders();}}));
}
