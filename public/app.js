const state = { token: localStorage.getItem('adminToken'), products: [], orders: [], fulfillmentOptions: [], orderView: 'active', orderStatus: 'all', orderQuery: '', activeTab: 'products', account: null, lineIntegration: null, lineStep: 1 };
...
async function loadLineIntegration(options={}){try{$('#lineIntegrationError').textContent='';const integration=await api('/api/admin/line-integration');$('#legacyLineNotice').hidden=!integration.legacy;$('#tenantLineSettings').hidden=integration.legacy===true;if(integration.legacy)return;renderLineIntegration(integration,options);}catch(error){$('#lineIntegrationError').textContent=error.message;}}
