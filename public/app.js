...
async function loadSheetsIntegration(){try{$('#sheetsIntegrationError').textContent='';const integration=await api('/api/admin/sheets-integration');state.sheetIntegration=integration;$('#legacySheetsNotice').hidden=!integration.legacy;$('#sheetsPlanLock').hidden=!integration.locked;$('#tenantSheetsSettings').hidden=integration.legacy===true||integration.locked===true;if(integration.legacy||integration.locked)return;$('#sheetsServiceEmail').value=integration.service_account_email||'';$('#sheetsUrl').value=integration.spreadsheet_url||'';$('#sheetsName').value=integration.sheet_name||'訂單';$('#sheetsEnabled').checked=integration.enabled===true;}catch(error){$('#sheetsIntegrationError').textContent=error.message;}}
...
$('#saveSheetsIntegration').addEventListener('click',async event=>{const button=event.currentTarget;button.disabled=true;button.textContent='測試連線中…';$('#sheetsIntegrationError').textContent='';try{const integration=await api('/api/admin/sheets-integration',{method:'PUT',body:JSON.stringify({enabled:$('#sheetsEnabled').checked,spreadsheet_url:$('#sheetsUrl').value,sheet_name:$('#sheetsName').value})});state.sheetIntegration=integration;toast(integration.enabled?'Google 試算表同步已啟用':'Google 試算表同步已停用');}catch(error){$('#sheetsIntegrationError').textContent=error.message;}finally{button.disabled=false;button.textContent='儲存並測試同步';}});
