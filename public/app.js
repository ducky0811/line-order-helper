const state = { token: localStorage.getItem('adminToken'), products: [], orders: [], report: null, fulfillmentOptions: [], orderView: 'active', orderStatus: 'all', orderQuery: '', orderDraftDirty: false, reportRange: 'month', activeTab: 'products', account: null, lineIntegration: null, lineStep: 1 };
...
function markOrderDraftDirty(event){if(event.target.matches('[data-message-text],[data-message-file],[data-quote-amount],[data-quote-note]'))state.orderDraftDirty=true;}
function shouldPauseOrderAutoRefresh(){const active=document.activeElement;const editingNow=active&&active.closest&&active.closest('#orderList')&&active.matches('textarea,input');const hasMessageDraft=[...document.querySelectorAll('[data-message-text]')].some(input=>input.value.trim());const hasSelectedFile=[...document.querySelectorAll('[data-message-file]')].some(input=>input.files&&input.files.length);const hasQuoteDraft=[...document.querySelectorAll('[data-quote-amount],[data-quote-note]')].some(input=>input.value.trim());return Boolean(state.orderDraftDirty||editingNow||hasMessageDraft||hasSelectedFile||hasQuoteDraft);}
...
setInterval(()=>{if(state.token&&state.activeTab==='orders'&&!document.hidden&&!shouldPauseOrderAutoRefresh())loadOrders();},10000);
