...
document.querySelectorAll('.tab').forEach(button=>button.addEventListener('click',()=>{
  ...
  if(selected==='orders') loadOrders(); if(selected==='settings'){loadSettings();loadLineIntegration();}
}));
...
async function loadLineIntegration(){try{const integration=await api('/api/admin/line-integration');$('#legacyLineNotice').hidden=!integration.legacy;$('#tenantLineSettings').hidden=integration.legacy===true;if(integration.legacy)return;$('#lineEnabled').checked=integration.enabled===true;$('#lineOfficialAccountId').value=integration.official_account_id||'';$('#lineChannelAccessToken').value='';$('#lineChannelSecret').value='';$('#lineWebhookUrl').value=integration.webhook_url||'';$('#lineBindCommand').value=integration.bind_code?`綁定店家 ${integration.bind_code}`:'';}catch(error){$('#lineIntegrationError').textContent=error.message;}}
$('#saveLineIntegration').addEventListener('click',async event=>{const button=event.currentTarget;button.disabled=true;$('#lineIntegrationError').textContent='';try{const integration=await api('/api/admin/line-integration',{method:'PUT',body:JSON.stringify({enabled:$('#lineEnabled').checked,official_account_id:$('#lineOfficialAccountId').value,channel_access_token:$('#lineChannelAccessToken').value,channel_secret:$('#lineChannelSecret').value})});$('#lineWebhookUrl').value=integration.webhook_url||'';$('#lineBindCommand').value=integration.bind_code?`綁定店家 ${integration.bind_code}`:'';$('#lineChannelAccessToken').value='';$('#lineChannelSecret').value='';toast(integration.enabled?'LINE 已啟用，請完成 Webhook 與店家綁定':'LINE 功能已停用');}catch(error){$('#lineIntegrationError').textContent=error.message;}finally{button.disabled=false;}});
