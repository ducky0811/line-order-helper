...
- 商家 Logo 與線上訂購封面圖直接上傳、預覽及恢復預設圖
- 一鍵暫停／恢復接單
- 店家 LINE 綁定與新訂單 Flex 通知（專業版）
...
- 伺服器端重新計價，避免客戶端竄改金額
- 狀態變更時主動通知 LINE 客戶
- 營業報表與 Excel 訂單匯出（專業版）
- 本機 JSON 測試模式與 Supabase 雲端模式
- 多店家 Email 註冊、專屬商店網址與 14 天試用
- 商品、訂單、設定與圖片依店家身分隔離
- 店家可自行選擇啟用 LINE，憑證加密儲存並使用專屬 Webhook
- 試用到期停止新訂單但保留後台資料，付費後依基本版／專業版自動切換功能
- 基本／試用方案顯示「Powered by 接單小幫手」平台品牌

## 方案功能

- 免費試用 14 天：可體驗專業版功能，資料保存 30 天。
- 基本版 NT$199／月：商品頁、商品管理、訂單與付款管理，資料保存 12 個月；不開放 LINE 通知與營業報表。
- 專業版 NT$399／月：基本版全部功能，加上 LINE 通知、營業報表、Excel 匯出與資料保存 3 年。
...
- `SUPPORT_LINE_URL`：店家後台「客服 LINE」按鈕連結，例如 `https://line.me/R/ti/p/@761grlvm`
