...
async function submitLast5(event){
  ...
  error.textContent='';
  try{
    const response=await fetch(`/api/shop/orders/${encodeURIComponent(code)}/payment`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({transfer_last5:last5})});
...
async function load(){
  ...
  try{
    const response=await fetch(`/api/shop/orders/${encodeURIComponent(code)}/status`);
