...
test('基本版與專業版權限及保存時間正確', () => {
  ...
  assert.deepEqual({ line: planCapabilities({ plan: 'pro' }).line, sheets: planCapabilities({ plan: 'pro' }).sheets, days: planCapabilities({ plan: 'pro' }).retention_days }, { line: true, sheets: true, days: 1095 });
