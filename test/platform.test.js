...
test('平台管理員可查看店家並開通專業版，店家權杖不能進入平台後台', async () => {
  ...
  try {
    ...
    const account = await fetch(`${base}/api/admin/account`, { headers: { Authorization: `Bearer ${merchant.token}` } }).then(response => response.json()); assert.equal(account.capabilities.sheets, false); assert.equal(account.merchant.plan, 'pro');
